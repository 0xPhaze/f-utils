...
contract TestFUtils is Test {
    ...
    function _testQuickPerm(uint256[] memory array) internal {
        uint256[] memory arraySorted = array.sort();
        uint256[][] memory out = array.quickPerm();

        for (uint256 i; i < out.length; i++) {
            for (uint256 j; j < out.length; j++) {
                if (i != j) {
                    assertFalse(out[i].eq(out[j]));
                    assertTrue(out[i].sort().eq(arraySorted));
                }
            }
        }
    }
    ...
    function testQuickPerm() public {
        _testQuickPerm([2, 1, 5].toMemory());
        _testQuickPerm([3, 1, 4, 5].toMemory());
        _testQuickPerm([9, 7, 3].toMemory());
    }
}
